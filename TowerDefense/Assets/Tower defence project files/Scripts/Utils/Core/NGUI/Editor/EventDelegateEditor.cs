...
public static class EventDelegateEditor
{
	...
	{
		return Field(undoObject, del, true, UISettings.minimalisticLook);
	}
	...
	static public bool Field (Object undoObject, EventDelegate del, bool removeButton, bool minimalistic)
	{
		...
		{
			if (!minimalistic) UIEditorTools.SetLabelWidth(82f);
			...
			if (ps != null)
			{
				for (int i = 0; i < ps.Length; ++i)
				{
					...
					if (selGO != null)
					{
						...
						int selection;
						string[] props = GetNames(ents, Core.NGUI.Utils.GetFuncName(param.obj, param.field), out selection);
	...
	static public string[] GetNames (List<Entry> list, string choice, out int index)
	{
		...
		{
			Entry ent = list[i];
			string del = Core.NGUI.Utils.GetFuncName(ent.target, ent.name);
			names[++i] = del;
	...
	{
		Field(undoObject, list, null, null, UISettings.minimalisticLook);
	}
