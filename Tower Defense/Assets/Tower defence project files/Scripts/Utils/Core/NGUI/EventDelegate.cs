...
// Copyright ｩ 2011-2015 Tasharen Entertainment
//----------------------------------------------
...
public class EventDelegate
{
    ...
    public bool Execute(Parameter[] parameters = null)
    {
#if !REFLECTION_SUPPORT
		if (isValid)
		{
        ...
        if (mMethod != null)
        {
            ...
            else
            {
                ...
                if (mArgs == null || mArgs.Length != parameters.Length)
                    mArgs = new object[parameters.Length];

                // Set all the parameters
                for (int i = 0, imax = parameters.Length; i < imax; ++i)
                    mArgs[i] = parameters[i].value;
    ...
    static public void Execute(List<EventDelegate> list, Parameter[] parameters = null)
    {
        if (list != null)
        {
    ...
    static public EventDelegate Add(List<EventDelegate> list, Callback callback, bool oneShot)
    {
        ...
        Debug.LogWarning("[JFGUtilities] [EventDelegate] Attempting to add a callback to a list that's null");
        return null;
    }
    ...
    static public void Add(List<EventDelegate> list, EventDelegate ev, bool oneShot)
    {
        ...
        else Debug.LogWarning("[JFGUtilities] [EventDelegate] Attempting to add a callback to a list that's null");
    }
