...
public abstract class UITweener : MonoBehaviour
{
    ...
    public static void Play(UITweener tweener, bool forward = true)
    {
        if (tweener == null)
        {
            return;
    ...
    public static void Play<TweenerType>(List<TweenerType> tweeners, bool forward = true) where TweenerType : UITweener
    {
        if (tweeners == null)
        {
            return;
    ...
    public static void ResetToBeginning(List<UITweener> tweeners)
    {
        if (tweeners == null)
        {
            return;
    ...
    /// <summary>
    /// How advanced or delayed is the tween?
    /// </summary>
    [HideInInspector]
    ...
    [HideInInspector]
    public event Action<UITweener> onFinishedAction;
    //[HideInInspector]
    //public event Action<UITweener> onFinishedLoopAction;

    // Deprecated functionality, kept for backwards compatibility
    [HideInInspector] public GameObject eventReceiver;
	[HideInInspector] public string callWhenFinished;

    public bool resetOnDsiable = false;
	...
	void Update()
	{
        ...
        if (((style == Style.Once) && (duration == 0f || mFactor > 1f || mFactor < 0f)) || ((style == Style.PingPongOnce) && mFactor < 0f))
		{
    ...
    protected virtual void OnFinishedLoop()
    {
    }
    ...
    ///// Play the tween.
    ///// </summary>
    //public void PlayFromStart() { Play(true, false); }
	...
	public virtual void Play() { Play(true, false); }

    public void PlayForwardAndReset()
    {
        Play(true, true);
    }
    public void PlayReverseAndReset()
    {
        Play(false, true);
    }
	...
	public void Play(bool forward, bool resetBeforePlaying = false)
	{
        ...
        {
            mAmountPerDelta = -mAmountPerDelta;
            mFactor = 1f; //Fix to play in reverse a yet to be played once tween!
            mFactor -= offsetFactor;
        }
        else
        {
	...
	{
		mStarted = false;
		//mFactor = (amountPerDelta < 0f) ? 1f - offsetFactor : 0f + offsetFactor;
        //Debug.Log("ResetToBeginning mFactor " + mFactor);
        mFactor = 0f;
    ...
    [ContextMenu("Debug_OnUpdate")]
    public void Debug_OnUpdate()
    {
        OnUpdate(0f, false);
    }
	...
	static public T Begin<T> (GameObject go, float duration) where T : UITweener
	{
		...
		comp.mFactor = 0f + comp.offsetFactor;
